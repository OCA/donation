...
class DonationSummarySend(models.TransientModel):
    _name = "donation.summary.send"
    _inherits = {"mail.compose.message": "composer_id"}
    _description = "Donation Summary Send"

    is_email = fields.Boolean(
        "Email", default=lambda self: self.env.company.invoice_is_email
    )
    summary_without_email = fields.Text(
        compute="_compute_donation_without_email",
        string="Summary(s) that will not be sent",
    )
    ...
    donation_summary_ids = fields.Many2many(
        "donation.summary",
        "donation_summary_donation_summary_send_rel",
        string="Donations Summary",
    )
    ...
    def _get_report_template(self):
        template = self.env.ref(
            "donation_summary.email_template_donation_summary", raise_if_not_found=False
        )
        return template.id or False

    @api.model
    def default_get(self, fields):
        res = super(DonationSummarySend, self).default_get(fields)
        res_ids = self._context.get("active_ids")

        donation_summary = self.env["donation.summary"].browse(res_ids)
        if not donation_summary:
    ...
    def _compute_composition_mode(self):
        for wizard in self:
            wizard.composer_id.composition_mode = (
                "comment" if len(wizard.donation_summary_ids) == 1 else "mass_mail"
            )

    def get_ref_report_name(self, wizard_template_id_lang):
        lang = (
            list(wizard_template_id_lang.values())[0]
            if wizard_template_id_lang
            else "XX"
        )
        report_names = {
    ...
    def onchange_template_id(self):
        for wizard in self:
            if wizard.composer_id:
                ...
                # res_ids = wizard.donation_summary_ids.ids
                # lang = wizard.template_id._render_template(lang_object, render_model, res_ids)
    ...
    def _compute_donation_without_email(self):
        for wizard in self:
            if wizard.is_email and len(wizard.donation_summary_ids) > 1:
                don_summary = self.env["donation.summary"].search(
                    [
                ...
                if don_summary:
                    ...
                    wizard.summary_without_email = False
            else:
                wizard.summary_without_email = False
    ...
    def _send_email(self):
        if self.is_email:
            ...
            for don_summary in self.donation_summary_ids:
                prioritary_attachments = don_summary.attachment_ids.filtered(
                    lambda x: x.mimetype.endswith("pdf")
                )
                if prioritary_attachments:
                    don_summary.with_context(tracking_disable=True).sudo().write(
    ...
    #     self.ensure_one()
    #     action = self.donation_summary_ids.action_print_tax_receipt()
    ...
    def send_and_print_action(self):
        ...
        self.donation_summary_ids.write({"state": "sent"})
    ...
    def save_as_template(self):
        ...
        action.update({"name": _("Send Summary")})
        return action

    def _print_document(self):
        """to override for each type of models that will use this composer."""
        self.ensure_one()
        action = self.donation_summary_ids.action_print_donation_summary()
        action.update({"close_on_report_download": True})
        return action
