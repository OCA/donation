...
class DonationSummary(models.Model):
    ...
    end_date = fields.Date(
        required=True, default=lambda self: self._default_end_date(),
        states={"sent": [("readonly", True)]}
    )
    ...
    def _compute_name(self):
        for rec in self:
            rec.name = (rec.partner_id.name).replace(" ", "") + "_" + str(rec.start_date).replace("-", "") + "_" + str(rec.end_date).replace("-", "") if rec.partner_id else ""
    ...
    def massive_creation(self):
        ...
        for partner in self.donation_ids.partner_id.ids:
            donations = self.donation_ids.filtered(lambda donation: donation.partner_id.id == partner)
    ...
    def massive_send_summary(self):
        ...
        compose_form = self.env.ref('donation_summary.donation_summary_send_wizard_form', raise_if_not_found=False)
        ctx = dict(
    ...
    def _get_mail_template(self):
        ...
        """
        return (
            'donation_summary.email_template_donation_summary'
        )
    ...
    def action_print_donation_summary(self):
        ...
        """
        self.filtered(lambda sum: not sum.is_summary_send).write({'is_summary_send': True})
        return self.env.ref('donation_summary.report_donation_summary').report_action(self)
