...
class TestDonationSale(TransactionCase):
    ...
    def test_sale_order(self):
        ...
        bank_journal = self.env['account.journal'].search([('type', '=', 'bank'), ('company_id', '=', self.invoice.company_id.id)], limit=1)
        payment_date = self.invoice.invoice_date + timedelta(days=5)
        wiz = self.env['account.payment.register'].with_context(active_model='account.move', active_id=self.invoice.id, active_ids=[self.invoice.id]).create({
        ...
        self.assertEqual(self.tax_receipt.partner_id, self.invoice.commercial_partner_id)
