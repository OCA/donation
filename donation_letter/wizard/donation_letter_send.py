...
class DonationLeterSend(models.TransientModel):
    ...
    donation_ids = fields.Many2many('donation.donation', 'donation_donation_donation_letter_send_rel', string='Donations')
    composer_id = fields.Many2one('mail.compose.message', string='Composer', required=True, ondelete='cascade')
    template_id = fields.Many2one('mail.template', 'Use template', domain="[('model', '=', 'donation.donation')]")
    ...
    def _compute_composition_mode(self):
        for wizard in self:
            wizard.composer_id.composition_mode = 'comment' if len(wizard.donation_ids) == 1 else 'mass_mail'
    ...
    def _send_email(self):
        if self.is_email:
            ...
            for donation in self.donation_ids:
                prioritary_attachments = donation.attachment_ids.filtered(lambda x: x.mimetype.endswith('pdf'))
