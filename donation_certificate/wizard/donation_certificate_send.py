...
class DonationCertificateSend(models.TransientModel):
    ...
    donation_without_email = fields.Text(compute='_compute_donation_without_email', string='Donation(s) that will not be sent')
    ...
    tax_receipt_ids = fields.Many2many('donation.tax.receipt', 'donation_tax_receipt_donation_certificate_send_rel', string='Donations Tax Receipt')
    composer_id = fields.Many2one('mail.compose.message', required=True, ondelete='cascade')
    template_id = fields.Many2one('mail.template', 'Use template', domain="[('model', '=', 'donation.tax.receipt'), ]", default=lambda self: self._get_report_template())

    def _get_report_template(self):
        template = self.env.ref('donation_certificate.email_template_donation_certificate', raise_if_not_found=False)
    ...
    def _compute_composition_mode(self):
        for wizard in self:
            wizard.composer_id.composition_mode = 'comment' if len(wizard.tax_receipt_ids) == 1 else 'mass_mail'
            ...
            if wizard.is_email and len(wizard.tax_receipt_ids) > 1:
                ...
                if tax_receipts:
                    wizard.donation_without_email = "%s\n%s" % (
                        _("The following donation(s) will not be sent by email, because the customers don't have email address."),
    ...
    def _send_email(self):
        if self.is_email:
            ...
            for tax_receipt in self.tax_receipt_ids:
                prioritary_attachments = tax_receipt.attachment_ids.filtered(lambda x: x.mimetype.endswith('pdf'))
                if prioritary_attachments:
                    tax_receipt.with_context(tracking_disable=True).sudo().write({'message_main_attachment_id': prioritary_attachments[0].id})
