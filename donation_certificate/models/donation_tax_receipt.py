...
class DonationTaxReceipt(models.Model):
    ...
    attachment_ids = fields.One2many(
        "ir.attachment",
        "res_id",
        domain=[("res_model", "=", "donation.tax.receipt")],
        string="Attachments",
    )
    amount_donation = fields.Monetary(tracking=True, compute="_compute_amounts")
    amount_donation_in_kind_consu = fields.Monetary(
        tracking=True, compute="_compute_amounts"
    )
    amount_donation_in_kind_service = fields.Monetary(
        tracking=True, compute="_compute_amounts"
    )
    ...
    def _compute_amounts(self):
        for rec in self:
            ...
            rec.amount_donation_in_kind_service = round(
                amount_donation_in_kind_service, 2
            )
    ...
    def action_print_tax_receipt(self):
        ...
        self.filtered(lambda inv: not inv.is_certificate_send).write(
