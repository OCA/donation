...
class DonationDonation(models.Model):
    ...
    number = fields.Char(
        related="move_id.name", readonly=True, store=True, string="Donation Number"
    )
    ...
    def _prepare_counterpart_move_line(
        self, name, amount_total_company_cur, total_amount_currency, currency_id
    ):
        self.ensure_one()
    ...
    def _prepare_donation_move(self):
        ...
        for donation_line in self.line_ids:
            ...
            ).product_id.product_tmpl_id._get_product_accounts()["income"]
            account_id = account.id
            analytic_account_id = donation_line.get_analytic_account_id()
            amount_currency = 0.0
            if (
                float_compare(
                    donation_line.amount_company_currency, 0, precision_digits=precision
                )
                == 1
            ):
    ...
    def validate(self):
        ...
        for donation in self:
            ...
            if float_is_zero(
                donation.amount_total, precision_rounding=donation.currency_id.rounding
            ):
                raise UserError(
                    _(
...
class DonationTaxReceipt(models.Model):
    ...
    def update_tax_receipt_annual_dict(
        ...
        for donation in donations:
            ...
            if float_is_zero(tax_receipt_amount, precision_rounding=precision_rounding):
                continue
