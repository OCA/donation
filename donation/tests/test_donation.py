...
class TestDonation(TransactionCase):
    ...
    def test_donation_1(self):
        ...
        for donation in donations:
            ...
            if donation == self.don4:  # full in-kind donation
                ...
                self.assertEquals(donation.journal_id, donation.move_id.journal_id)
                self.assertEquals(donation.donation_date, donation.move_id.date)
            if donation.tax_receipt_option == "each" and donation.tax_receipt_total:
                ...
                self.assertEquals(donation.donation_date, tax_receipt.donation_date)
                self.assertEquals(donation.tax_receipt_total, tax_receipt.amount)
    ...
    def test_annual_tax_receipt(self):
        ...
        partner_familly = self.res_partner.create(
            {"name": u"Famille Joly", "tax_receipt_option": "annual"}
        )
        partner_husband = self.res_partner.create(
            {"parent_id": partner_familly.id, "name": u"Xavier Joly"}
        )
        partner_wife = self.res_partner.create(
            {"parent_id": partner_familly.id, "name": u"Stéphanie Joly"}
        )
        ...
        self.assertEquals(tax_receipt.date, fields.Date.from_string(last_day_year))
        self.assertEquals(
            tax_receipt.donation_date, fields.Date.from_string(last_day_year)
        )
        self.assertEquals(tax_receipt.currency_id, dons[0].company_id.currency_id)
